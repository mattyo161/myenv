...
| File | Purpose |
| --- | --- |
| `group_vars/all.yml` | Shared package lists and settings (taps, formulae, casks, mise tools). |
| `group_vars/local.yml` | Your identity (name, email, GitHub user). Git-ignored; generated by `bootstrap.sh`. |
...
defined once and updates both the apps and the dotfiles — no duplication.
